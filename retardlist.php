...
<body>
    ...
    <?php
        ...
        echo "<h1><div><span>Voici la liste de tous déja en retard :</span></h1>";
        echo "<p>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspNombre total :&nbsp"
    ?>
    <?php
        include('conn.php');
        $query=mysqli_query($conn,"select count(ret_id) as total from `t_retard`");
    ...
    <?php
        ...
        echo "<h1><div><span>Voici la liste de tous élèves jamais en retard :</span></h1>";
        echo "<p>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspNombre total :&nbsp"
    ?>
    <?php
        include('conn.php');
        $query=mysqli_query($conn,"select count(cla.cla_id) as total from t_classe cla right join (a_classe cl inner join (t_eleve el left join t_retard ret on el.el_id = ret.el_id) on cl.el_id = el.el_id) on cla.cla_id = cl.cla_id where ret.ret_date is NULL;");
        $row=mysqli_fetch_array($query);
    ?>
    <?php echo $row['total'],"</div>"; 
        ?>
    <?php 
        require_once("conn.php");
        $req = "select ret.ret_id as id,ret.ret_date as date,el.el_nom as nom,el.el_prenom as prenom,cla.cla_nom as classe,ret.ret_motif as motif from t_classe cla right join (a_classe cl inner join (t_eleve el left join t_retard ret on el.el_id = ret.el_id) on cl.el_id = el.el_id) on cla.cla_id = cl.cla_id having ret.ret_date is NULL;";
        $resultat = mysqli_query($conn,$req) or die(mysql_error());
    ?>
    <div>
    <table>
